...
#dg
open (OUT, '> outer.gaps');         #Open a file called 'outer' for outputing.
...
print "Input Match Reward: ";
$V = <>;
print "Input Space Penalty(as positive #): ";
$Im = <>;
print "Input Mismatch Penalty(as positive #): ";
$Cm = <>;
print "Input Gap Penalty(as positive #): "; #check if gap < space
$Gm = <>;
while ($Gm > $Im){
    $Gm = <>;
}
...
#  print OUT "For $i, $j, t is $t \n"; # Another debugging line.
  if ($max < $V[$i][$j-1] - $Im){
     if ($prevmove eq 'vertical space'){
             $max = $V[$i][$j-1] - $Gm;
             $prevmove = 'vertical gap';
             #vertical space
     }
     
     elsif ($prevmove eq 'vertical gap'){
             $max = $V[$i][$j-1];
             $prevmove = 'vertical gap';
             #vertical gap
         }
         ...
         $max = $V[$i][$j-1] - $Im;
         $prevmove = 'vertical space';
     }
    }
     
  if ($V[$i-1][$j] - $Im > $max) {
      if ($prevmove eq 'horizontal space'){
          $max = $V[$i-1][$j] - $Gm;
          $prevmove = 'horizontal gap';
          #vertical space
      }
      
      elsif ($prevmove eq 'horizontal gap'){
          $max = $V[$i-1][$j];
          $prevmove = 'horizontal gap';
          #vertical gap
      }
